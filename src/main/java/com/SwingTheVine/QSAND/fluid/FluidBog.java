...
/** Implements Bog fluid block calculation and physics.
 * 
 * @since <b>0.49.0</b>
...
public class FluidBog extends SinkingBlockFluidClassic implements IMetaBlockName {
	...
	// public static final PropertyInteger SINK = PropertyInteger.create("sink", 0, Integer.valueOf(types.length-1)); // Creates a
	// metadata value for every "types" metadata value
	...
	public FluidBog(final Fluid fluid, final Material material) {
		...
		final int opacity = (ConfigHandler.makeBlocksOpaque) ? 32 : 3;
	...
	public void breakBlock(final World world, final BlockPos pos, final IBlockState state) {
		...
		if (state.getBlock().getMetaFromState(state) == 0 && world.getBlockState(pos).getBlock() != this) {
			...
			if (world.getBlockState(pos).getBlock().getMaterial().isLiquid()) {
				...
				final int denToRep = getDensity(world, pos);
	...
	public void onEntityCollidedWithBlock(final World world, final BlockPos pos, final IBlockState state,
		...
		if (triggeringEntity instanceof EntityLivingBase) {
			...
			triggEntitySplashing = (triggeringEntity.motionY < -0.1) // TODO: Mud Changed. Simplified equation
				? true
				: false;
			...
			if (triggEntityRotating || Math.abs(triggeringEntity.prevPosX - triggeringEntity.posX) > 0.0015
				|| Math.abs(triggeringEntity.prevPosZ - triggeringEntity.posZ) > 0.0015) { // TODO: Mud Changed. 0.001 -> 0.0015
			...
			if (world.getTotalWorldTime() % 128L == 0L
				...
				if (triggEntitySplashing && triggEntityPrevSunk_kof2 > 1.5) { // TODO: Mud Changed. logic changed
					...
					if (triggeringEntity instanceof EntityPlayer && world.rand.nextInt(3) == 0) {
						// QuicksandManager.spawnBodyBubbleRandom(world, triggeringEntity, pos.getX(), pos.getY(), pos.getZ(),
				...
				if (!triggEntitySplashing && triggEntityMoving) {
					...
					if (world.rand.nextInt(2) == 0) {
						world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
							"game.player.swim", 0.15f, world.rand.nextFloat() * 0.15f + 0.1f); // TODO: Mud Changed Play sound
					}
			...
			if ((world.getBlockState(pos.up(1)) != this && world.getBlockState(pos.up(1)) != QSAND_Blocks.moss)
				|| ((world.getBlockState(pos.up(1)) == this || world.getBlockState(pos.up(1)) == this)
					&& (world.getBlockState(pos.up(2)) == this
						|| world.getBlockState(pos.up(2)) == QSAND_Blocks.moss))) { // TODO: Mud Changed. Added moss checks
				...
				if (triggEntitySunk_kof1 >= 1.2) {
					...
					if (triggEntitySunk_kof1 < 1.35) { // TODO: Mud Changed. Added statement
						...
						if (!triggEntitySplashing) {
							...
							if (triggEntityBootsFloat) {
								...
								// Basically, "thicknessLower" is the maximum value, and the further the player has sunk, the
								// closer it approaches 0.
								// After a while (happens sooner the higher the punishment value is), the rate it approaches 0
								// changes and becomes faster
								// https://www.desmos.com/calculator/klpmbgz7je
								beacon.logBeacon("MotionY", "6"); // TODO: Mud Changed. Major change. Changed equation.
								triggeringEntity.motionY += thicknessLower / Math.max((movementPunish_mofKofDiv - 1.0)
									* (Math.max(triggEntitySunkMod_kof1m - 0.5, 0.75) * 1.6 * blockMetadataBumped)
									* 17.5, 1.0) / Math.pow(Math.max(triggEntitySunkMod_kof1m / 1.25, 1.0), 2.0);
								beacon.logBeacon("Equation", "2",
									thicknessLower
										/ Math.max((movementPunish_mofKofDiv - 1.0)
											* (Math.max(triggEntitySunkMod_kof1m - 0.5, 0.75) * 1.6
												* blockMetadataBumped)
											* 17.5, 1.0)
							...
							if (triggeringEntity instanceof EntityPlayer) {
								...
								if (world.getTotalWorldTime()
									% Math.max(
										1.0 + Math
											.floor(Math.max(triggEntitySunk_kof1 * 20.0 - blockMetadata / 10, 0.0)),
										1.0) == 0.0) { // TODO: Mud Changed. Major equation change
								...
								triggeringEntity.setPosition(
									triggeringEntity.prevPosX
										+ (triggeringEntity.posX - triggeringEntity.prevPosX) / mw_kof,
									triggEntityPosY, triggeringEntity.prevPosZ
										+ (triggeringEntity.posZ - triggeringEntity.prevPosZ) / mw_kof);
							}
							...
							if (world.rand.nextInt(Math.max(
								(int) Math.floor(5.0f - Math.pow(Math.max(triggEntitySunkMod_kof1m, 0.0), 1.5)),
								1)) == 0) {
					...
					if (triggEntityPosY - triggEntityPrevPosY > 0.001
						&& world.rand.nextInt(Math.max((int) Math.floor(triggEntitySunkMod_kof1m * 7.5), 15)) == 0
						&& QuicksandManager.suctionWorldCheck(triggeringEntity, world, triggEntityVelY)) { // TODO: Mud Changed.
																											// 3.0 -> 7.5 10 -> 15
						triggeringEntity.motionY -= 0.025 * blockMetadataBumped
					...
					if (triggEntitySunk_kof1 >= 0.9) {
						...
						// This triggers more frequently the higher the metadata value is. (1/7 to 1/4 chance)
						final double thicknessLower = (world.getTotalWorldTime()
							% Math.max(8.0f - blockMetadataBumped * 2, 1.0f) == 0.0f) // TODO: Mud Changed. Multiplied
																						// blockMetadataBumped by 2
								? 0.07485
								: 0.075 + sinkRateMod_mys;
								
						// EQUATION BEACON 3. 2nd complex
						triggeringEntity.motionY += thicknessLower / Math.max(
							(movementPunish_mofKofDiv - 1.0)
								* (Math.max(triggEntitySunkMod_kof1m - 0.5, 0.75) * 1.6 * blockMetadataBumped) * 17.5,
							1.0) / Math.pow(Math.max(triggEntitySunkMod_kof1m / 1.25, 1.0), 2.0); // TODO: Mud Changed. Same major
																									// change as before
						beacon.logBeacon("Equation", "3",
							thicknessLower / Math.max((movementPunish_mofKofDiv - 1.0)
								* (Math.max(triggEntitySunkMod_kof1m - 0.5, 0.75) * 1.6 * blockMetadataBumped) * 17.5,
							1.0) / Math.pow(Math.max(triggEntitySunkMod_kof1m / 1.25, 1.0), 2.0));
						...
						if (!triggEntitySplashing) { // TODO: Mud Changed. Added statement
							...
							if (world.rand.nextInt(Math.max(
								(int) Math.floor(5.0f - Math.pow(Math.max(triggEntitySunkMod_kof1m, 0.0), 1.5)),
								1)) == 0) { // TODO: Mud Changed. Removed bumpedMetadata, 4.0 -> 5.0
								// Happens more frequently the more the entity is sunk in the block.
								// However, this only triggers when the entity is sunk more than 1.2 blocks
								// That means this only happens on metadata 4 (Bottomless Mud).
						...
						// If the entity is moving upwards, AND the number in the world's random number generator sequence
						// modified by the modified distance sunk by the triggering entity equals 0, AND the entity has been
						// marked for suction...
						if (triggEntityPosY - triggEntityPrevPosY > 0.001
							&& world.rand.nextInt(Math.max((int) Math.floor(triggEntitySunkMod_kof1m * 7.5), 15)) == 0
							&& QuicksandManager.suctionWorldCheck(triggeringEntity, world, triggEntityVelY)) { // TODO: Mud
																												// Changed. 3.0 ->
																												// 7.5 10 -> 15
						...
						final double thicknessHigher = (world.getTotalWorldTime() % (int) Math.floor(Math.min(16.0,
						...
						if (world.rand.nextInt(Math.max(
							(int) Math.floor(
								25.0f * blockMetadataBumped - Math.pow(Math.max(triggEntitySunkMod_kof1m, 0.0), 1.5)),
							1)) == 0) { // TODO: Mud Changed. Major equation change
							// Happens more frequently the more the entity is sunk in the block.
							// However, this only triggers when the entity is sunk more than 1.2 blocks
							// Metadata 0... (Mud)
							// 0 has a 1/12 chance
						...
						if (triggEntitySunk_kof1 >= 0.5) {
							...
							if (world.rand.nextInt((int) Math
								...
								if (!triggEntityMoving) {
									...
									triggeringEntity.motionY += (0.0725 + sinkRateMod_mys) * thicknessHigher
										/ (movementPunish_mofKofDiv + 0.01); // TODO: Mud Changed. 0.025 -> 0.01
								}
							} // ...if the entity is NOT marked as moving...
							...
							if (!triggEntityMoving) {
								...
								beacon.logBeacon("MotionY", "11.2",
									(0.0725 + sinkRateMod_mys) * thicknessHigher / (movementPunish_mofKofDiv + 0.01));
								// Bumps the entity downwards slightly.
							...
							if (triggEntitySunk_kof1 < 0.45
								...
								QuicksandManager.setStuckEffect((EntityLivingBase) triggeringEntity, 100); // TODO: Mud Changed.
																											// 145 -> 100
							}
						}
					}
			...
			QuicksandManager.antiHoldJumpScript(triggeringEntity, triggEntitySunk_kof1, true); // TODO: Mud Changed. Moved from
																								// above
			/*
	...
	public int checkFusion(final World world, final BlockPos pos, final Random rand) {
		...
		for (int re = 0; re <= 6; ++re) {
			...
			if (world.getBlockState(new BlockPos(xx, yy, zz)).getBlock().getMaterial() == Material.lava) {
				met = world.getBlockState(new BlockPos(xx, yy, zz)).getBlock()
					.getMetaFromState(world.getBlockState(new BlockPos(xx, yy, zz)));
				isLavaNear = true;
				break;
			}
		...
		if (isLavaNear) {
			...
			for (int l = 0; l < 8; ++l) {
				world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, pos.getX() + Math.random(), pos.getY() + 1.2,
					pos.getZ() + Math.random(), 0.0, 0.0, 0.0);
			}
			return 2;
		}
		return 1;
	...
	public void updateTick(final World world, final BlockPos pos, final IBlockState state, final Random rand) {
		...
		if (state.getBlock().getMetaFromState(state) == 0) {
			final Block block = world.getBlockState(new BlockPos(pos.getX(), pos.getY() + this.densityDir, pos.getZ()))
				.getBlock();
			final int bMeta = world.getBlockState(new BlockPos(pos.getX(), pos.getY() + this.densityDir, pos.getZ()))
				.getBlock().getMetaFromState(
					world.getBlockState(new BlockPos(pos.getX(), pos.getY() + this.densityDir, pos.getZ())));
			if (block == this && bMeta != 0) {
				final BlockPos tempPos = new BlockPos(pos.getX(), pos.getY() + this.densityDir, pos.getZ());
			...
			if (world.rand.nextInt(5) == 0) {
				if (world.getBlockState(pos.add(1, 0, 0)).getBlock() == this) {
					++inThisBlocks;
					if (world.getBlockState(pos.add(1, 0, 0)).getBlock()
						.getMetaFromState(world.getBlockState(pos.add(1, 0, 0))) == 0) {
						++inMetaBlocks;
					}
				}
				if (world.getBlockState(pos.add(-1, 0, 0)).getBlock() == this) {
				...
				if (world.getBlockState(pos.add(0, 0, -1)).getBlock() == this) {
					++inThisBlocks;
					if (world.getBlockState(pos.add(0, 0, -1)).getBlock()
						.getMetaFromState(world.getBlockState(pos.add(0, 0, -1))) == 0) {
						++inMetaBlocks;
					}
				}
				if (world.getBlockState(pos.add(0, 0, 1)).getBlock() == this) {
				...
				if (inThisBlocks > 2 && inMetaBlocks > 1) {
					// world.setBlockMetadataWithNotify(pos,
					// world.getBlockState(pos).getBlock().getMetaFromState(world.getBlockState(pos)) - 1, 2);
					// TODO: Make sure this works properly. It is so janky...
					world.setBlockState(pos, world.getBlockState(pos).getBlock().getStateFromMeta(
						world.getBlockState(pos).getBlock().getMetaFromState(world.getBlockState(pos)) - 1), 2);
				}
			}
		}
	...
	public void checkPlayerMuddy(final EntityPlayer triggeringPlayer, final int blockPosX, final int blockPosY,
		...
		if (!world.isRemote) {
			...
			if (preMudLevel * (this.maxOpacity / 1000.0f) > playerMudControl.getMudLevel()
				* (playerMudControl.getMudTime() / 1000.0f)) {
				playerMudControl.setMudLevel(preMudLevel);
				final int mdtp = QuicksandManager.getMudType(this);
	...
	public String getSpecialName(final ItemStack stack) {
		...
		return null; // FluidBog.types[stack.getItemDamage()];
	}
