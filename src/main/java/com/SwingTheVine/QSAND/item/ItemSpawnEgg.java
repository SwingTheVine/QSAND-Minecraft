 ...
 * @author Jabelar - 1.7.10 <a href="https://web.archive.org/web/20190529171948/http://jabelarminecraft.blogspot.com/p/minecraft-forge-1721710-creating-custom.html">source code</a>
 */
public class ItemSpawnEgg extends ItemMonsterPlacer {
	...
	public ItemSpawnEgg() {
        ...
        this.setCreativeTab(QSAND.QSANDTab); // Places this item in the custom creative tab
    ...
    public void getSubItems(Item defaultEgg, CreativeTabs tab, List<ItemStack> subItems) {
		
		// For every registered egg...
        for (String eggName : EntityRegistry.getEggs().keySet()) {
        	
        	ItemStack itemStackEgg = new ItemStack(defaultEgg); // Creates a new instance of the default egg item
            NBTTagCompound nbtTag = new NBTTagCompound(); // Creates a new NBT Tag
            nbtTag.setString("entity_name", eggName); // Adds the name of the mob to spawn (the name of the mob to spawn is derived from its egg which has the same name)
            itemStackEgg.setTagCompound(nbtTag); // Adds the NBT Tag onto the default egg. This makes it a subtype/metadata variant of the default egg
            subItems.add(itemStackEgg); // Adds the subtype/metadata variant of the default egg to the list of subtypes for this item
        }
    }
     ...
     * @See {@link ItemMonsterPlacer#addInformation Super Implementation}
	 */
	@Override
	public void addInformation(final ItemStack item, final EntityPlayer player, final List tooltip, final boolean inAdvancedMode) {
