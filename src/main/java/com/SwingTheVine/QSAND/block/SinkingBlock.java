...
/** Extends the {@link Block} class whilst implementing {@link IMetaBlockName} to gain access to tooltips.
 * <p>
...
public class SinkingBlock extends Block implements IMetaBlockName {
	...
	public SinkingBlock(final Material material) {
		super(material); // Runs all of the code in the super implementation of this constructor
	}
	...
	/** {@inheritDoc} */
	@Override
	public void setTooltip(final ItemStack item, final EntityPlayer player, final List tooltips,
		final boolean isAdvanced) {
		
		// Since this function is supposed to be unique to every block, we do nothing
	}
	 ...
	 * 
	 * @param state - The current state of the block. Used for returning different colors for each metadata/subtype of the block
	 * @return RGB value in integer form
	 * @see {@link ColorManager} for converting the RGB integer into human readable formats */
	@SideOnly(Side.CLIENT)
	...
	/** The color to modify the base quicksand block color by.
	 * <p>
	 * "Tints" the block color.
	 * 
	 ...
	 * <p>
	 * This functions calls the {@link SinkingBlock#getQuicksandBlockColor() getQuicksandBlockColor} function.
	 * 
	 * @param world - The world the block is in
	 * @param pos - The coordinates of the block
	 * @param renderPass - TODO: Figure out what this is and why 0 is the default value
	...
	/** Obtains a String array of all the special names of the quicksand block.
	 * <p>
	 * Not to be confused with the {@link SinkingBlock#getSpecialName(ItemStack) getSpecialName} function
	 * which returns only a single metadata/subtype special name.
	 * 
	 * @return All possible metadata/subtype special names of the quicksand block
	 * @see {@link SinkingBlock#getSpecialName(ItemStack) getSpecialName(ItemStack)} which returns only one metadata/subtype
	 * special name */
