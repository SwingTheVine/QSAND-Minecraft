...
/** Implements Larvae block calculation and physics.
 * 
 * @since <b>0.45.0</b>
...
public class BlockLarvae extends SinkingBlock implements IMetaBlockName {
	...
	// Constructor
	public BlockLarvae(final Material material) {
		super(material);
	...
	public void onEntityCollidedWithBlock(final World world, final BlockPos pos, final IBlockState state,
		...
		if (triggeringEntity instanceof EntityLivingBase) {
			...
			final int mr_blackgoo = (int) Math.min(
				5.0 + Math.floor(Math.max(0.0, blockMetadataBumped * 100.0f * (1.5 - triggEntitySunkMod_kof1m))),
				128.0);
			...
			triggEntityJumping = (triggEntityPosY - triggEntityPrevPosY > 0.1) ? true : false;
			...
			if ((triggEntityMoving
				...
				if (triggEntitySplashing && triggEntityPrevSunk_kof2 > 1.5) {
					world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
						"mob.silverfish.step", 0.5f, world.rand.nextFloat() * 0.25f + 0.25f);
				}
				if (!triggEntitySplashing && triggEntityMoving && world.rand.nextInt(2) == 0) {
					world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
						"mob.silverfish.step", 0.25f, world.rand.nextFloat() * 0.15f + 0.1f);
				}
				if (triggEntityJumping) {
					world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
						"mob.silverfish.step", 0.75f, world.rand.nextFloat() * 0.15f + 0.85f);
				}
			}
			...
			if (world.getBlockState(pos.up(1)) != this
				...
				if (triggEntitySunk_kof1 >= 1.2) {
					...
					triggeringEntity.motionY += thickness
						/ Math.max(
							...
							1.0)
						/ Math.pow(Math.max(triggEntitySunk_kof1 / 1.25, 1.0), 2.0);
					triggeringEntity.onGround = false;
					triggeringEntity.fallDistance = 0.0f;
					
					if (world.rand.nextInt(Math
						.max((int) Math.floor(15.0 - Math.pow(Math.max(triggEntitySunk_kof1, 0.0), 1.5)), 1)) == 0) {
					...
					if (triggEntitySunk_kof1 > 0.9) {
						...
						if (triggeringEntity instanceof EntityPlayer) {
							...
							if (world.getTotalWorldTime() % Math.max(
								1.0 + Math.floor(Math.max(triggEntitySunk_kof1 * 5.0 - blockMetadata / 10, 0.0)),
								1.0) == 0.0) {
						...
						if (world.rand.nextInt(Math.max(
							(int) Math.floor(15.0 - Math.pow(Math.max(triggEntitySunk_kof1, 0.0), 1.5)), 1)) == 0) {
							triggeringEntity.onGround = true;
						}
					} else {
						...
						if (triggEntitySunk_kof1 >= 0.6) {
							...
							if (triggEntityMoving || world.rand
								.nextInt(Math.max((int) Math.floor(triggEntitySunk_kof1 * 150.0), 25)) == 0) {
								
								if (!triggEntityMoving) {
									triggeringEntity.motionY += thickness / Math.max(2.0, 5.0 - triggEntitySunk_kof1);
								} else if (world.rand.nextInt(Math
									.min(Math.max((int) Math.floor(75.0 - triggEntitySunk_kof1 * 25.0), 1), 10)) == 0) {
									triggeringEntity.motionY += thickness / Math.max(2.0, 5.0 - triggEntitySunk_kof1);
								} else {
									triggeringEntity.motionY += thickness;
								}
							...
							if (triggEntityMoving || world.rand
								...
								if (!triggEntityMoving) {
									...
									.min(Math.max((int) Math.floor(75.0 - triggEntitySunk_kof1 * 25.0), 1), 35)) == 0) {
					...
					if (!world.isRemote) {
						...
						if (triggEntitySunk_kof1 < 0.75 && triggEntityPosY - triggEntityPrevPosY < -0.005) {
							triggeringEntity.attackEntityFrom(DamageSource.generic,
								Math.max(((EntityLivingBase) triggeringEntity).getMaxHealth() * 0.05f, 1.0f));
							world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
								"mob.silverfish.step", 0.25f, world.rand.nextFloat() * 0.15f + 0.1f);
							world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
								"mob.silverfish.hit", 0.25f, world.rand.nextFloat() * 0.25f + 1.75f);
							...
							if (triggEntitySunk_kof1 < 0.01) {
								...
								.nextInt(Math.max((int) Math.floor(triggEntitySunk_kof1 * 100.0), 10)) == 0) {
								triggeringEntity.attackEntityFrom(DamageSource.generic,
									Math.max(((EntityLivingBase) triggeringEntity).getMaxHealth() * 0.05f, 1.0f));
								world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
									"mob.silverfish.step", 0.25f, world.rand.nextFloat() * 0.15f + 0.1f);
						...
						if (world.rand.nextInt(Math.max((int) Math.floor(triggEntitySunk_kof1 * 50.0), 10)) == 0) {
							world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
								"mob.silverfish.say", 0.25f, world.rand.nextFloat() * 0.25f + 1.75f);
						}
					}
				}
				...
				if (!world.isRemote) {
					...
					if (world.getBlockState(pos.up(2)).getBlock() == this) {
						...
						if (world.rand.nextInt(10) == 0) {
							triggeringEntity.attackEntityFrom(DamageSource.generic,
								((EntityLivingBase) triggeringEntity).getMaxHealth() * 0.05f);
							world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
								"mob.silverfish.step", 0.25f, world.rand.nextFloat() * 0.15f + 0.1f);
							world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
								"mob.silverfish.hit", 0.25f, world.rand.nextFloat() * 0.25f + 1.75f);
			...
			if (triggEntitySunk_kof1 < 1.25) {
				...
				if (triggeringEntity instanceof EntityItem
					&& ((EntityItem) triggeringEntity).getEntityItem().getItem() instanceof ItemFood
					&& world.rand.nextInt(Math.max((int) Math.floor(triggEntitySunk_kof1 * 50.0), 10)) == 0) {
					world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
						"mob.silverfish.say", 0.25f, world.rand.nextFloat() * 0.25f + 1.75f);
	...
	public void checkLavaNear(final World world, final BlockPos pos) {
		...
		for (int re = 0; re <= 6; ++re) {
			...
			if (world.getBlockState(new BlockPos(xx, yy, zz)).getBlock().getMaterial() == Material.lava) {
				met = world.getBlockState(new BlockPos(xx, yy, zz)).getBlock().getDamageValue(world,
		...
		if (isLavaNear || isFireNear) {
			...
			if (world.rand.nextInt(5) == 0) {
				world.playSoundEffect(pos.getX() + world.rand.nextFloat(), pos.getY() + world.rand.nextFloat(),
					pos.getZ() + world.rand.nextFloat(), "mob.silverfish.hit", 0.5f,
					world.rand.nextFloat() * 0.25f + 1.75f);
			}
	...
	@Override
	public void harvestBlock(final World world, final EntityPlayer entityPlayer, final BlockPos pos,
		final IBlockState state, final TileEntity tileEntity) {
		
		if (EnchantmentHelper.getSilkTouchModifier(entityPlayer)) {
			QuicksandManager.dropItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(this));
	...
	public void breakBlock(final World world, final BlockPos pos, final IBlockState state) {
		...
		if (this != world.getBlockState(pos)) {
			world.playSoundEffect(pos.getX() + world.rand.nextFloat(), pos.getY() + world.rand.nextFloat(),
				pos.getZ() + world.rand.nextFloat(), "mob.spider.death", 0.75f, world.rand.nextFloat() * 0.25f + 1.75f);
		}
	...
	public void onNeighborChange(final IBlockAccess world, final BlockPos pos, final BlockPos neighbor) {
		...
		((World) world).scheduleBlockUpdate(pos, this, this.tickRate((World) world), 1);
	}
