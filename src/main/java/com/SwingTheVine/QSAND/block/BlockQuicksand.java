...
/** Implements Quicksand block calculation and physics.
 * 
 * @since <b>0.13.0</b>
...
public class BlockQuicksand extends SinkingBlock implements IMetaBlockName {
	...
	// Constructor
	public BlockQuicksand(final Material material) {
		super(material);
	...
	public void onEntityCollidedWithBlock(final World world, final BlockPos pos, final IBlockState state,
		...
		final int blockMetadata = 0; // Obtains this block's variant/metadata. TODO: Changed. Since there is only one type, this
										// is 0
		...
		if (triggeringEntity instanceof EntityLivingBase) {
			...
			final int mr_blackgoo = (int) Math.min(
				5.0 + Math.floor(Math.max(0.0, blockMetadataBumped * 10.0f * (1.5 - triggEntitySunkMod_kof1m))), 145.0); // TODO:
																															// Changed
			...
			triggEntitySplashing = (triggeringEntity.motionY < -0.1) // TODO: Changed. Lack of metadata has simplified this
																		// statement
				? true
				: false;
			...
			if (triggEntityRotating || Math.abs(triggeringEntity.prevPosX - triggeringEntity.posX) > 0.0015
				|| Math.abs(triggeringEntity.prevPosZ - triggeringEntity.posZ) > 0.0015) {
			...
			if (world.getTotalWorldTime() % 128L == 0L
				...
				if (triggEntitySplashing) {
					...
					if (triggEntityPrevSunk_kof2 > 1.5) {
						...
						world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ,
							"game.player.swim", 0.25f, world.rand.nextFloat() * 0.25f + 0.1f); // Play sound
					}
					
					// If the entity is a player, AND the number in the world's random number generator sequence is 0 (1/2
					// chance)...
				...
				if (triggEntityJumping) {
					...
					// If the entity is a player, AND the number in the world's random number generator sequence equals 0 (1/2
					// chance)...
					// TODO: Changed. 5 -> 2
					if (triggeringEntity instanceof EntityPlayer && world.rand.nextInt(2) == 0) {
						QuicksandManager.spawnBodyBubbleRandom(world, triggeringEntity, pos.getX(), pos.getY(),
			...
			if (world.getBlockState(pos.up(1)) != this
				...
				if (triggEntityJumping
					...
					triggeringEntity.motionY -= 0.05 * (Math.min(0.75, triggEntitySunkMod_kof1m) + 1.0); // Subtract some unknown
																											// modifier to the
																											// entity's Y velocity
				...
				if (triggEntitySunk_kof1 >= 1.2) {
					...
					if (!triggEntitySplashing) {
						...
						final double thicknessLower = (world.getTotalWorldTime() % 4L == 0L) ? 0.07485 : 0.075;
						
						// EQUATION BEACON. 2nd complex
						// Makes the entity sink extremely fast
						// This equation is really complicated.
						// Basically, "thicknessLower" is the maximum value, and the further the player has sunk, the closer it
						// approaches 0.
						// After a while (happens sooner the higher the punishment value is), the rate it approaches 0 changes and
						// becomes faster
					...
					if (triggEntitySunk_kof1 >= 0.9) {
						...
						final double thicknessLower = (world.getTotalWorldTime() % 2L == 0L) ? 0.0748 : 0.075;
						...
						if (world.rand
							.nextInt((int) Math.floor(Math.max(triggEntitySunkMod_kof1m * 5.0 / 2.0, 1.0))) == 0) {
						...
						double thicknessHigher = (world.getTotalWorldTime()
							% (int) Math.floor(Math.min(4.0, Math.max(8.0 - triggEntitySunk_kof1 * 8.0, 1.0))) == 0L)
								? 1.01
								: 1.05;
						...
						if (triggEntitySunk_kof1 >= 0.5) {
							...
							if (world.rand.nextInt((int) Math
								...
								if (!triggEntityMoving) {
									...
									triggeringEntity.motionY += (0.0725 + sinkRateMod_mys) * thicknessHigher
										/ (movementPunish_mofKofDiv + 0.075);
								}
							} // ...if the entity is NOT marked as moving...
								...
								triggeringEntity.motionY += (0.075 + sinkRateMod_mys) * thicknessHigher
									/ (movementPunish_mofKofDiv + 0.075); // Unknown modifer to entity's Y velocity
							}
						} else {
	...
	public void randomDisplayTick(final World world, final int x, final int y, final int z, final Random random) {
		...
		if (world.getBlockState(new BlockPos(x, y + 1, z)).getBlock().getMaterial() == Material.air) {
			final int md = world.getBlockState(new BlockPos(x, y, z)).getBlock()
				.getMetaFromState(world.getBlockState(new BlockPos(x, y, z)));
