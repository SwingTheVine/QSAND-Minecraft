...
import com.SwingTheVine.QSAND.entity.ai.EntityAISlimeMud;
...
/** An entity that controls Mud Slimes
 * 
 * @since <b>0.30.0</b>
...
public class EntitySlimeMud extends EntityLiving implements IMob {
	...
	public static final String textureLocation = "textures/entity/slime/Mud.png"; // The location of the texture used for the
																					// bubble
	public int datawatcherDepthID = 13; // Default datawatcher ID
	public int datawatcherSizeID = 14; // Default datawatcher ID
	
	// Constructor
	public EntitySlimeMud(final World world) {
		super(world);
		...
		this.moveHelper = new EntityAISlimeMud.SlimeMoveHelper(this);
		this.targetTasks.addTask(1, new EntityAIFindEntityNearestPlayer(this));
		this.tasks.addTask(2, new EntityAISlimeMud.AISlimeAttack(this));
		this.tasks.addTask(3, new EntityAISlimeMud.AISlimeFaceRandom(this));
		this.tasks.addTask(4, new EntityAISlimeMud.AISlimeHop(this));
	}
	...
	protected EntitySlimeMud createInstance() {
		
		return new EntitySlimeMud(this.worldObj);
	}
		...
		if (this.onGround && !isOnGround) {
			...
			for (int slimeSize = this.getSlimeSize(), particle = 0; particle < slimeSize * 16; particle++) {
				...
				this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, true, this.posX + offsetX,
					...
					Block.getIdFromBlock(QSAND_Blocks.mud));
			}
		...
		if (this.riddenByEntity == null) {
			...
			if (this.deepFactor < 12.5f) {
				...
				if (initialPullTime <= 0.0f) {
					...
					this.pullTime = 35 + this.rand.nextInt(10); // Set pullTime to a number between 35 and 44
	...
	public void onCollideWithPlayer(final EntityPlayer player) {
		...
		if (this.canDamagePlayer()) {
			...
			if (!(player.ridingEntity instanceof EntitySlimeVoid) && !(player.ridingEntity instanceof EntitySlimeMud)
				...
				&& !(player.ridingEntity instanceof EntitySlimeMud)) {
	...
	@Override
	public boolean attackEntityFrom(final DamageSource dmgs, final float power) {
						...
						if (power < 3.0f * opMul) {
							...
							for (int i = 0; i < this.slimeInv.length; ++i) {
								...
								if (this.slimeInv[i] == null) {
									...
									this.checkPlayerMuddy((EntityPlayer) ply, 0.0,
										ply.posY - 0.25 + this.rand.nextFloat() * 0.5f, 0.0, this.worldObj);
									Stopper = true;
