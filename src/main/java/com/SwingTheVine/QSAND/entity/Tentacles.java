...
public class Tentacles extends Entity implements IEntityAdditionalSpawnData {
	...
	public Tentacles(final World world) {
        super(world);
    ...
    public Tentacles(final World world, final double entityPosX, final double entityPosY, final double entityPosZ, final Entity target, final int high, final int animationLifeTime) {
        this(world);
    ...
    public void CheckInQuicksandBlock() {
        if (this.worldObj.getBlockState(new BlockPos((int)Math.floor(this.posX), (int)Math.floor(this.posY - 0.5), (int)Math.floor(this.posZ))) != QSAND_Blocks.swallowingFlesh
        		&& this.worldObj.getBlockState(new BlockPos((int)Math.floor(this.posX), (int)Math.floor(this.posY + 0.5), (int)Math.floor(this.posZ))) != QSAND_Blocks.swallowingFlesh) {
            this.setDead();
        }
    }
    ...
    public void DeadEffect() {
        for (int var1 = 0; var1 < 5; ++var1) {
            ...
            this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + this.rand.nextFloat() * this.width * 2.0f - this.width, this.high + 0.5f + (double)(this.rand.nextFloat() * this.height), this.posZ + this.rand.nextFloat() * this.width * 2.0f - this.width, var2, var3, var4);
        }
        this.setDead();
    }
    ...
    public boolean attackEntityFrom(final DamageSource dmgs, final float amount) {
        ...
        if (this.health > 0.0f && this.maxHurtResistantTime < 1) {
            ...
            if (this.health <= 0.01f) {
                this.playSound("game.neutral.die", 1.0f, (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) * 0.2f + 1.0f);
                this.playSound("mob.silverfish.kill", 1.0f, (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) * 0.2f + 1.75f);
                this.playSound("mob.silverfish.step", 0.5f, this.worldObj.rand.nextFloat() * 0.25f + 0.5f);
                this.health = 0.0f;
                this.checkTarget();
            } else {
                this.playSound("game.neutral.hurt", 1.0f, (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) * 0.2f + 1.0f);
            }
