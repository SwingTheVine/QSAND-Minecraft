...
public class EntityLongStick extends Entity implements IEntityAdditionalSpawnData{
    ...
    public void spawnParticle(final Block block, final int metadata, final int count, final double radius, final double radiusY, final double offsetY) {
    	
    	for (int currentParticle = 0; currentParticle < count; currentParticle++) {
    		this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + this.worldObj.rand.nextFloat() * radius - radius / 2.0, this.posY + offsetY + this.worldObj.rand.nextFloat() * radiusY - radiusY / 2.0, this.posZ + this.worldObj.rand.nextFloat() * radius - radius / 2.0, 0.0, 0.0, 0.0);
    	}
    }
    
    public void playSound(final String sound, final float volume, final float pitch, final float rpitch) {
    	this.worldObj.playSound(this.posX, this.posY, this.posZ, sound, volume, this.worldObj.rand.nextFloat() * rpitch + pitch, false);
    }
    
    public void playBottom(final Block block) {
    	
    	if (block.getMaterial().isSolid()) {
    		this.worldObj.playSound(this.posX, this.posY - 1.0, this.posZ, block.stepSound.getStepSound(), 0.2f, 1.0f, false); // TODO: getPitch is gone
    	}
    }
    
    public void playSuction(final float volume) {
    	this.worldObj.playSound(this.posX, this.posY, this.posZ, "mob.magmacube.jump", volume / 2.5f, 0.25f + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4f, false);
    	this.worldObj.playSound(this.posX, this.posY, this.posZ, "mob.magmacube.jump", volume / 2.5f, 0.25f + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4f, false);
    	this.worldObj.playSound(this.posX, this.posY, this.posZ, "mob.magmacube.jump", volume / 2.5f, 0.25f + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4f, false);
    	this.worldObj.playSound(this.posX, this.posY, this.posZ, "mob.magmacube.jump", volume / 2.5f, 0.25f + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4f, false);
    }
