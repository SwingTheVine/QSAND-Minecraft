...
/** Controls calculations for bubbles that appear in quicksand/mud.
 * 
 * @since <b>0.24.0</b>
...
public class EntityBubble extends Entity {
	...
	// Constructor
	public EntityBubble(final World world) {
		super(world);
	...
	public EntityBubble(final World world, final double bubblePosX, final double bubblePosY, final double bubblePosZ,
		...
		for (int i = 0; i <= 5; ++i) {
			...
			if (!world.getBlockState(new BlockPos(blockPosX, (int) Math.floor(bubblePosY + 0.5 + i), blockPosZ))
				.getBlock().getMaterial().isOpaque()
				&& !world.getBlockState(new BlockPos(blockPosX, (int) Math.floor(bubblePosY + 0.5 + i), blockPosZ))
		...
		final int riseToBlockPosY = (int) Math.floor(maximumSpawningHeight - 0.5); // The Y position of the highest block the
																					// bubble can rise to
		
		// If the block the bubble rises to is NOT a liquid, AND the block is not in its default state (0 metadata)...
		if (world.getBlockState(new BlockPos(blockPosX, riseToBlockPosY, blockPosZ)).getBlock().getMaterial().isLiquid()
			&& world.getBlockState(new BlockPos(blockPosX, riseToBlockPosY, blockPosZ)).getBlock()
				.getMetaFromState(world.getBlockState(new BlockPos(blockPosX, riseToBlockPosY, blockPosZ))) != 0) {
			this.setDead(); // Kills the bubble
		}
	...
	public void onUpdate() {
		...
		// If the remainder of the total world time divided by 32 equals 0, AND the block half a block below this bubble is NOT
		// the same as the block that spawned the bubble...
		if (this.worldObj.getTotalWorldTime() % 32L == 0L
			&& this.worldObj.getBlockState(new BlockPos((int) Math.floor(this.posX), (int) Math.floor(this.posY - 0.5),
				(int) Math.floor(this.posZ))) != this.block) {
		...
		if (Sys.getTime() - this.entitySpawnTime > this.entityLiveTime) {
			...
			this.worldObj.playSound(this.posX, this.posY, this.posZ, "liquid.lavapop",
				0.5f + this.worldObj.rand.nextFloat() * 0.25f,
				(0.5f + this.worldObj.rand.nextFloat() * 0.5f) / this.size, false);
			this.worldObj.playSound(this.posX, this.posY, this.posZ, "mob.slime.attack", 0.15f,
				(1.25f + this.worldObj.rand.nextFloat() * 0.5f) / this.size, false);
