...
import com.SwingTheVine.QSAND.entity.item.EntityLongStick;
...
/** Manages all things related to quicksand, sinking, quicksand type, etc.
 * 
 * @since <b>0.23.0</b>
...
public class QuicksandManager {
	...
	public static void dropItem(final World world, final int x, final int y, final int z, final ItemStack item) {
		...
		if (!world.isRemote) {
			...
			world.spawnEntityInWorld(entityitem); // Spawns the item into the world
		}
	}
	
	// Spawns in a new item in an exact position
	public static void dropItem(final World world, final double x, final double y, final double z,
	...
	public static boolean isEntityInsideOfBlock(final Entity triggeringEntity, final Block block) {
		...
		final Block triggEntityInsideBlockLower = (block == QSAND_Blocks.moss)
			? triggeringEntity.worldObj
				.getBlockState(new BlockPos(triggEntityPosX, triggEntityMaxPosY + 1, triggEntityPosZ)).getBlock()
			: triggeringEntity.worldObj
				.getBlockState(new BlockPos(triggEntityPosX, triggEntityMaxPosY, triggEntityPosZ)).getBlock();
				
		// Same as before except it is checking one block higher
		final Block triggEntityInsideBlockHigher = (block == QSAND_Blocks.moss)
			? triggeringEntity.worldObj
				.getBlockState(new BlockPos(triggEntityPosX, triggEntityMaxPosY + 2, triggEntityPosZ)).getBlock()
			: triggeringEntity.worldObj
				.getBlockState(new BlockPos(triggEntityPosX, triggEntityMaxPosY + 1, triggEntityPosZ)).getBlock();
		...
		return triggEntityInsideBlockLower == block && (block == QSAND_Blocks.moss
			|| (triggEntityInsideBlockHigher.getMaterial().isSolid() && triggEntityMaxPosY + 1 > triggEntityHeight)
			|| triggEntityMaxPosY + triggEntityInsideBlockLower.getBlockBoundsMaxY() > triggEntityHeight);
	}
	...
	public static boolean isEntityInsideOfBlockM(final Entity triggeringEntity, final Block block) {
		...
		final Block triggEntityInsideBlockLower = triggeringEntity.worldObj
			.getBlockState(new BlockPos(triggEntityPosX, triggEntityMaxPosY + 1, triggEntityPosZ)).getBlock();
	...
	public static boolean isEntityInsideOfBlockL(final Entity triggeringEntity, final Block block) {
		...
		if (triggEntityInsideBlockLower == block) {
			return triggEntityPosZ + triggEntityInsideBlockLower.getBlockBoundsMaxY() > triggEntityHeight
				|| isEntityInsideOfBlockS(triggeringEntity, block);
		}
	...
	public static int getLastMudType(final int type) {
		...
		try {
			...
			return (Integer) QSAND_Blocks.blockObjectList[(type * 5) + 3];
	...
	public static boolean suctionWorldCheck(final Entity entity, final World world, final double velocity) {
		...
		return world.getTotalWorldTime() % Math.floor(Math.max(10.0 * Math.pow(velocityY, 2.0), 1.0)) == 0.0
	...
	public static void antiHoldJumpScript(final Entity entity, final double triggEntitySunk, final boolean isStuck) {
			...
			if ((EntityPlayer) entity == Minecraft.getMinecraft().thePlayer) {
				...
				if (ConfigHandler.useCustomArmorCalc) {
					...
					for (int armorSlot = 0; armorSlot <= 3; armorSlot++) {
						...
						if (((EntityPlayer) entity).getCurrentArmor(armorSlot) != null
							&& ((EntityPlayer) entity).getCurrentArmor(armorSlot).getItem() instanceof ItemArmor) {
							
							// Finds how much damage the boots reduce
							damageReduction = ((ItemArmor) ((EntityPlayer) entity).getCurrentArmor(armorSlot).getItem())
								.getArmorMaterial().getDamageReductionAmount(3 - armorSlot);
								
							// Adds the calculated weight of this piece of armor into the total armor weight variable
							weightArmor += Math.signum(Math.max(((-0.4 * armorSlot) + 1.3) - triggEntitySunk, 0.0))
								* (damageReduction + 10.0 / Math.max(damageReduction, 1.0)) / 2.0
								+ Math.pow(damageReduction, 2.0);
						}
					}
				}
					...
					if (entity.worldObj.getTotalWorldTime() % 32L == 0L) {
						...
						for (int invSlot = 0; invSlot <= 35; invSlot++) {
							...
							if (((EntityPlayer) entity).inventory.mainInventory[invSlot] != null) {
								final ItemStack slotItem = ((EntityPlayer) entity).inventory.mainInventory[invSlot]; // The item
																														// in the
																														// slot
								...
								if (slotItem.getItem() instanceof ItemBlock) {
									...
									if (slotBlock != null) {
										...
										if (slotBlock.hasTileEntity(slotBlock.getDefaultState())) {
											weightInv += 10 * slotItem.stackSize; // Add the number of items in that slot times 10
																					// as the weight
										} else {
											final Material slotBlockMaterial = slotBlock.getMaterial(); // Get the material of the
																										// block in the slot
											weightInv += (2.5 + (slotBlockMaterial.getCanBurn() ? 0 : 1) * 2.5
												+ (slotBlock.canDropFromExplosion((Explosion) null) ? 1 : 0)
													* slotBlock.getExplosionResistance((Entity) null) / 2.0)
												/ ((slotBlock.isOpaqueCube() ? 0 : 1) + 1.0)
												* (((slotBlockMaterial.getMaterialMobility() == 2) ? 1 : 0) + 1.0) / 2.0
												* slotItem.stackSize;
										}
									} else {
										weightInv += 0.5 * slotItem.stackSize; // Adds the number of items in that slot times 0.5
																				// as the weight
									}
									...
									weightInv += Math
										.pow(
											((ItemArmor) slotItem.getItem()).getArmorMaterial()
												.getDamageReductionAmount(((ItemArmor) slotItem.getItem()).armorType),
										2.0) * slotItem.stackSize;
								} else if (slotItem.getItem().isDamageable()) {
									// Else if the item can be damaged (i.e. sword)
									
									weightInv += 5 * slotItem.stackSize; // Adds the weight of the number of items in the slot
																			// times 5
								} else if (slotItem.getItem().getItemStackLimit(slotItem) == 1) {
									// Else if the item can NOT be stacked (i.e. bow)
									
									weightInv += 2.5 * slotItem.stackSize; // Adds the weight of the number of items in the slot
																			// times 2.5
								} else {
									// Else...
									
									weightInv += 0.5 * slotItem.stackSize; // Adds the weight of the number of items in the slot
																			// times 0.5
								}
							}
						}
						
						ConfigHandler.serverInstanceWeightInv = weightInv; // Stores the inventory weight of the player in the
																			// server
					}
				}
				...
				weight = Math.max(
					-250.0 + weightInv / 2.5 * (1.0 + Math.signum(Math.max(0.75 - triggEntitySunk, 0.0)) * 1.5), 0.0)
					+ weightArmor;
		...
		if (isStuck && ConfigHandler.useCustomBootCalc) {
			...
			// If the entity is wearing boots, AND the entity has sunk less than 1.475 blocks into this one, AND the boot don't
			// float...
			if (hasBoots && triggEntitySunk < 1.475 && !bootsFloat) {
				...
				entity.onGround = (false | entity.isCollidedVertically); // If the entity is colliding with something on the Y
																			// axis, then the entity is marked as on the ground
				
				// If the remainder of the total world time divided by an equation equals 0...
				if (entity.worldObj.getTotalWorldTime()
					% Math.max(1.0 + Math.floor(20.0 / Math.max(triggEntitySunk * 5.0, 0.01)), 1.0) == 0.0) {
				...
				if (entity.worldObj.isRemote) {
					...
					final PlayerStuckEffectManager playerProperties = PlayerStuckEffectManager
						.get((EntityLivingBase) entity);
					...
					setStuckEffect((EntityLivingBase) entity,
						Math.max(Math.max(Math.min(
							(int) Math.floor(Math.pow(10.0 * Math.max(1.5 - triggEntitySunk, 0.0), 2.0)) + stuckLevel,
							255), 5), stuckLevel));
				}
			}
		}
	...
	public static boolean isJumpKeyDown(final EntityPlayer player) {
		...
		if (player.worldObj.isRemote && Minecraft.getMinecraft().thePlayer == player) {
			...
			return GameSettings.isKeyDown(Minecraft.getMinecraft().gameSettings.keyBindJump); // Returns true if the player is
																								// pressing down the key binded to
																								// their jump key
		}
	...
	public static boolean isCrouchKeyDown(final EntityPlayer player) {
		...
		if (player.worldObj.isRemote && Minecraft.getMinecraft().thePlayer == player) {
			final GameSettings gameSettings = Minecraft.getMinecraft().gameSettings; // The game settings of the player
			return GameSettings.isKeyDown(Minecraft.getMinecraft().gameSettings.keyBindSneak); // Returns true if the player is
																								// pressing down the key binded to
																								// their sneak key
		}
	...
	public static Entity getEntityByUUID(final World world, final Entity entity, final UUID uuid, final double radius) {
		...
		final AxisAlignedBB boundingBox = AxisAlignedBB.fromBounds(entity.posX - radius, entity.posY - radius,
			entity.posZ - radius, entity.posX + radius, entity.posY + radius, entity.posZ + radius);
	...
	public static void spawnMudTentacles(final World world, final Entity entity, final int blockPosX,
		final int blockPosY, final int blockPosZ, final Block block, final int metadata, final int rate,
		final int chance) {
		...
		if (ConfigHandler.useMudTentacles && !world.isRemote && entity != null && entity instanceof EntityLivingBase
			&& entity.width < 1.75 && entity.isCollidedVertically && world.getTotalWorldTime() % rate == 0L
			...
			if (effectWeak != null) {
				effectWeakLevel = effectWeak.getAmplifier(); // Obtain the amplification level on the player for this potion
																// effect
			}
			
			// If the slow effect is NOT null...
			if (effectSlow != null) {
				effectSlowLevel = effectSlow.getAmplifier(); // Obtain the amplification level on the player for this potion
																// effect
			}
			...
			if (effectWeakLevel <= -1 || effectSlowLevel <= 2) {
				...
				world.spawnEntityInWorld(new EntityTentaclesMud(world, entity.posX,
					Math.min(entity.posY + 0.5, blockPosY + 1), entity.posZ, entity, blockPosY + 1, 0));
			}
		}
	}
	
	// Handler for mud tentacles
	public static void handleMudTentacles(final World world, final Entity entity, final int blockPosX,
		final int blockPosY, final int blockPosZ, final Block block, final int metadata) {
		...
		if (ConfigHandler.useMudTentacles && !world.isRemote && entity != null && entity instanceof EntityItem
			&& ((EntityItem) entity).getEntityItem().getItem() instanceof ItemFood && !entity.isCollidedVertically
			&& ((ItemFood) ((EntityItem) entity).getEntityItem().getItem()).isWolfsFavoriteMeat()
			&& world.getTotalWorldTime() % 64L == 0L && world.rand.nextInt(5) == 0
			&& canBeMudTentacles(world, blockPosX, blockPosY, blockPosZ, block, metadata)) {
			...
			world.spawnEntityInWorld(new EntityLongStick(world, entity.posX, blockPosY + 1, entity.posZ, (Entity) null,
				blockPosX, blockPosY, blockPosZ));
		}
	}
	
	public static boolean canBeMudTentacles(final World world, final int blockPosX, final int blockPosY,
		final int blockPosZ, final Block block, final int metadata) {
		...
		if (ConfigHandler.useMudTentacles && !world.isRemote) {
			...
			final BlockPos blockPos = new BlockPos(blockPosX, blockPosY, blockPosZ);
			
			// If the block above this block is NOT the same as this block, AND all other adjacent blocks are the same as this
			// block
			if (world.getBlockState(blockPos.up(1)) != block && world.getBlockState(blockPos.down(1)) == block
				...
				int spawnChance = 10; // Base spawn chance of mud tentacles being able to spawn. Smaller equals greater spawn
										// chance
	...
	public static void spawnBubbleDelay(final World world, final double blockPosX, final double blockPosY,
		...
		world.spawnEntityInWorld(
			new EntityBubble(world, blockPosX, blockPosY, blockPosZ, block, metadata, size, time, delay));
	}
	...
	public static void spawnBodyBubble(final World world, final Entity entity, final int blockPosX, final int blockPosY,
		...
		if (world.getBlockState(new BlockPos((int) Math.floor(blockPosX), blockPosY, (int) Math.floor(blockPosZ)))
			.getBlock() != block) {
		...
		if (useMetadata) {
			blockMetadata = world.getBlockState(new BlockPos(blockPosX, blockPosY, blockPosZ)).getBlock()
				.getMetaFromState(world.getBlockState(new BlockPos(blockPosX, blockPosY, blockPosZ)));
		}
	...
	public static void spawnQSBubble(final World world, final double x, final double y, final double z,
		final Block block, final int metadata, final float volume) {
		...
		if (!ConfigHandler.spawnBubbles) {
			if (volume > 0.4f) {
				for (int i = 0; i < 3; ++i) {
					world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, x, y, z, 0.0, 0.0, 0.0,
		...
		world.playSound(x, y, z, "liquid.lavapop", volume + world.rand.nextFloat() * 0.25f,
			0.25f + world.rand.nextFloat() * 0.5f, false);
	}
	...
	public static void spawnDrowningBubble(final World world, final Entity entity, final SinkingBlock block,
		...
		// If the entity is a player, AND the entity has disabled damage, AND the user does NOT want to spawn singleplayer
		// bubbles...
		if (entity instanceof EntityPlayer && ((EntityPlayer) entity).capabilities.disableDamage
			&& !ConfigHandler.spawnUnseenBubbles) {
			// The user will never see these bubbles normally in singleplayer.
			// There is no reason to spawn them UNLESS they are recording using a mod
		...
		if (world.getTotalWorldTime() % 48L == 0L && world.rand.nextInt(2) == 0) {
			...
			for (int i = 0; i < 4 + world.rand.nextInt(6); i++) {
				...
				if (entity instanceof EntityPlayer) {
					bubblePosY -= 0.15; // The bubble will spawn 0.15 blocks lower (since the eye level is 0.15 blocks lower for
										// players)
				}
	...
	public static void spawnStickBubble(final World world, final double x, final double y, final double z,
		final SinkingBlock block, final boolean useMetadata, final boolean deep) {
		...
		for (int i = 0; i < random; ++i) {
			...
			// Spawns the bubble
			spawnBubbleDelay(world, bubblePosX, blockPosY + surfaceY(block), bubblePosZ, block, blockMetadata, size,
				time, i * 200 + world.rand.nextInt(10) * 100);
		}
	}
	...
	public static void addCustomDrownAir(final Entity entity, final int modifier) {
		...
		try {
			...
			if (entity instanceof EntityPlayer) {
				...
				entity.getDataWatcher().updateObject(ConfigHandler.customDrownAirPlayersDW,
					(Object) (entity.getDataWatcher().getWatchableObjectInt(ConfigHandler.customDrownAirPlayersDW)
						+ modifier));
			} else {
				
				// If the entity is NOT a player, it adds the modifier to the datawatcher for custom drowning for entities
				entity.getDataWatcher().updateObject(ConfigHandler.customDrownAirEntitiesDW,
					(Object) (entity.getDataWatcher().getWatchableObjectInt(ConfigHandler.customDrownAirEntitiesDW)
						+ modifier));
			}
		} catch (final Exception ignored) {
		}
	...
	public static boolean isChildOf(final Class parent, final Class child) {
		...
		return child.getSuperclass() == parent
			|| (child.getSuperclass() != Entity.class && isChildOf(parent, child.getSuperclass()));
	}
