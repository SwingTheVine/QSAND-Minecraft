...
import com.SwingTheVine.QSAND.item.ItemLongStick;
...
/** Handles custom air/drowning events for quicksand.
 * 
 * @since <b>0.32.0</b>
...
public class CustomAirHandler {
			...
			if (event.entity instanceof EntityPlayer) {
				...
				try {
					...
					if (ConfigHandler.customDrownAirPlayersDWEcp < 5) {
						...
						final CrashReport report = CrashReport.makeCrashReport(exception,
							"Error initializing Oxygen System. Muddy Air DataWatcher ID conflict: "
								+ ConfigHandler.customDrownAirPlayersDW + " with entity player ");
				...
				try {
					...
					if (ConfigHandler.customDrownAirEntitiesDWEcp < 5) {
						...
						final CrashReport report = CrashReport.makeCrashReport(exception,
							"Error initializing Oxygen System. Muddy Mobs Air DataWatcher ID conflict: "
								+ ConfigHandler.customDrownAirEntitiesDWEcp + " with entity living ");
						throw new ReportedException(report);
					}
				}
			}
	...
	public void onLivingUpdateEvent(final LivingEvent.LivingUpdateEvent event) {
			...
			if (player.worldObj.isRemote) {
				...
				if (player != Minecraft.getMinecraft().thePlayer) {
					...
					if (player.getCurrentEquippedItem() != null
						&& player.getCurrentEquippedItem().getItem() == QSAND_Items.longStick) {
		...
		if (event.entityLiving.worldObj.isRemote) {
			...
			final int gma = QuicksandManager.getCustomDrownAir(event.entityLiving);
			...
			final Block[] failIfEntityInside = {QSAND_Blocks.mireLiquid, QSAND_Blocks.mireLiquidStable,
				QSAND_Blocks.larvae, QSAND_Blocks.meatHole, QSAND_Blocks.voidHole};
			...
			for (int currentBlockIndex = 0; currentBlockIndex < failIfEntityInside.length; currentBlockIndex++) {
				...
				if ((QuicksandManager.isEntityInsideOfBlock(event.entityLiving,
					failIfEntityInside[currentBlockIndex]))) {
					return;
				}
			}
			...
			for (int currentBlockIndex = 0; currentBlockIndex < CustomPlayerGUIRenderer.blockList.length; currentBlockIndex++) {
				...
				if (QuicksandManager.isEntityInsideOfBlockM(event.entityLiving, QSAND_Blocks.moss)) {
					
					if (gma > -1) {
						QuicksandManager.addCustomDrownAir(event.entityLiving, -1);
					}
					
					return;
				} else if (QuicksandManager.isEntityInsideOfBlock(event.entityLiving,
					CustomPlayerGUIRenderer.blockList[currentBlockIndex])) {
			...
			if (gma < 300) {
				QuicksandManager.addCustomDrownAir(event.entityLiving, Math.min(5, 300 - gma));
			}
		}
	}
