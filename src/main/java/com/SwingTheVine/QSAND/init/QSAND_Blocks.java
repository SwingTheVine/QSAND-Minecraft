...
public class QSAND_Blocks {
	...
	public static SinkingBlock test_block;
	...
	public static void init() {
		...
		snowSoft = (SinkingBlock) new BlockSnowSoft(Material.ground).setUnlocalizedName("snow_soft")
			.setCreativeTab(QSAND.QSANDTab);
		quicksand = (SinkingBlock) new BlockQuicksand(Material.ground).setUnlocalizedName("quicksand")
			.setCreativeTab(QSAND.QSANDTab);
		larvae = new BlockLarvae(Material.coral).setUnlocalizedName("larvae").setCreativeTab(QSAND.QSANDTab);
		mud = (SinkingBlock) new BlockMud(Material.ground).setUnlocalizedName("mud").setCreativeTab(QSAND.QSANDTab);
		test_block = (SinkingBlock) new BlockTest(Material.ground).setUnlocalizedName("test_block")
			.setCreativeTab(QSAND.QSANDTab);
	}
	...
	public static void registerRenders() {
		...
		registerRenderInventory(snowSoft, ((BlockSnowSoft) snowSoft).getTypes(),
			((BlockSnowSoft) snowSoft).getUseOneTexture());
		registerRenderInventory(quicksand, ((BlockQuicksand) quicksand).getTypes(),
			((BlockQuicksand) quicksand).getUseOneTexture());
		...
		registerRenderInventory(test_block, ((BlockTest) test_block).getTypes(),
			((BlockTest) test_block).getUseOneTexture());
	}
	
	// Registers the inventory image for all block variants
	public static void registerRenderInventory(final Block block, final String[] types, final boolean useOneTexture) {
		
		final Item item = Item.getItemFromBlock(block); // The item variant of the block
		OreDictionary.registerOre("quicksandBlock_" + block.getUnlocalizedName().substring(5),
			new ItemStack(block, 1, OreDictionary.WILDCARD_VALUE)); // Registers the block to the ore dictionary for creative tab
																	// sorting
		...
		if (useOneTexture) {
			...
			for (int indexMeta = 0; indexMeta < types.length; indexMeta++) {
				Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(item, indexMeta,
			...
			for (int indexMeta = 0; indexMeta < types.length; indexMeta++) {
				Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(item, indexMeta,
					new ModelResourceLocation(
						ModInfo.id + ":" + item.getUnlocalizedName().substring(5) + "_" + types[indexMeta],
						"inventory"));
			}
		}
	}
