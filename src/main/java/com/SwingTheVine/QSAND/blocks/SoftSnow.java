...
public class SoftSnow extends Block implements IMetaBlockName {
	...
	public void onEntityCollidedWithBlock(World world, BlockPos pos, IBlockState state, Entity triggeringEntity) {
		...
		final int blockMetadata = state.getValue(SINK).intValue(); // Obtains this block's variant/metadata. TODO: Changed. Same as mud
		...
		if (triggeringEntity instanceof EntityLivingBase) {
			...
			final int mr_blackgoo = (int)Math.min(5.0 + Math.floor(Math.max(0.0, blockMetadataBumped * 10.0f * (1.5 - triggEntitySunkMod_kof1m))), 60.0); // TODO: Changed Quicksand 145 -> 60
			...
			triggEntityJumping = 
					(triggEntityPosY - triggEntityPrevPosY > 0.1) // Changed. Quicksand 0.2 -> 0.1
					? true : false;
			...
			if (triggEntityRotating || Math.abs(triggeringEntity.prevPosX - triggeringEntity.posX) > 0.001 || Math.abs(triggeringEntity.prevPosZ - triggeringEntity.posZ) > 0.001) {
				...
				movementPunish_mofKofDiv = 1.0 + triggEntityMovingDistance_movDis / 2.0; // TODO: Quicksand Changed * 25.0 -> / 2.0
			...
			if ((triggEntityMoving && world.getTotalWorldTime() % Math.max((int)Math.floor(forceBubbleSpawn_movCof / 1.5), 1) == 0L) 
				...
				if (triggEntitySplashing && triggEntityPrevSunk_kof2 > 1.5) {
					world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ, "step.snow", 1.0f, world.rand.nextFloat() * 0.5f + 0.5f);
				}
				
				if (!triggEntitySplashing && triggEntityMoving && world.rand.nextInt(2) == 0) {
                    world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ, "dig.snow", 1.0f, world.rand.nextFloat() * 0.5f + 0.5f);
                }
				
                if (triggEntityJumping) {
                    world.playSoundEffect(triggeringEntity.posX, triggEntityPosY, triggeringEntity.posZ, "dig.cloth", 0.75f, world.rand.nextFloat() * 0.5f + 0.5f);
                }
			}
	        ...
	        if (world.getBlockState(pos.up(1)) != this || (world.getBlockState(pos.up(1)) == this && world.getBlockState(pos.up(2)) == this)) {
                ...
                if (triggEntitySunk_kof1 >= 1.2) {
            		...
            		double thicknessLower = 0.085 + sinkRateMod_mys;
            		...
            		// TODO: Quicksand Changed. Added sink rate modifer back and 0.085 mod straight
            		double thicknessHigher = 0.085 + sinkRateMod_mys;
                    ...
                    if (triggEntitySunk_kof1 >= 0.6) { // TODO: Quicksand changed. 0.5 -> 0.6
                        ...
                        if (world.rand.nextInt(Math.min(Math.max((int)Math.floor(75.0 - triggEntitySunk_kof1 * 25.0), 1), 75)) == 0) {
                            ...
                            } else if (world.rand.nextInt(Math.min(Math.max((int)Math.floor(75.0 - triggEntitySunk_kof1 * 25.0), 1), 10)) == 0) { // TODO: Quicksand added
                    	...
                    	if (triggEntityMoving || world.rand.nextInt(Math.min(Math.max((int)Math.floor(75.0 - triggEntitySunk_kof1 * 25.0), 1), 75)) == 0) {
	                        ...
	                        if (triggEntitySunk_kof1 < 0.45 && QuicksandManager.isTrulySinking(triggeringEntity, triggEntitySunk_kof1)) {
	                        	QuicksandManager.setStuckEffect((EntityLivingBase)triggeringEntity, 145); // TODO: Quicksand changed. 145 -> 60
	                        }
                    	}
                    }
                }
                ...
                if (world.getTotalWorldTime() % Math.max(1.0 + Math.floor(Math.max(triggEntitySunk_kof1 * 10.0, 0.0)), 1.0) == 0.0) {
                    triggeringEntity.onGround = false;
                }
	...
	public String getSpecialName(ItemStack stack) {
		return SoftSnow.types[stack.getItemDamage()];
	}
	
	// Returns the block's metadata through damage value
	public int getDamageValue(World world, int blockPosX, int blockPosY, int blockPosZ) {
		return world.getBlockState(new BlockPos(blockPosX, blockPosY, blockPosZ)).getBlock().getDamageValue(world, new BlockPos(blockPosX, blockPosY, blockPosZ));
	}
	...
	public void harvestBlock(World world, EntityPlayer player, BlockPos pos, IBlockState state, TileEntity te) {
        if (EnchantmentHelper.getSilkTouchModifier((EntityLivingBase)player)) {
            QuicksandManager.dropItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack((Block)this, 1, state.getBlock().damageDropped(state)));
