...
/** Handles the Mud Datawatcher for the player.
 * <p>
 * This holds information like the color of the skin overlay.
 * 
 * @since <b>0.40.0</b>
...
public class PlayerMudManager implements IExtendedEntityProperties {
	...
	public PlayerMudManager(final EntityPlayer player) {
		...
		try {
			...
			final CrashReport report = CrashReport.makeCrashReport(e,
				"Error initializing skin overlay. There appears to be a conflict between IDs of the local and global mud levels. Currently \""
					+ ConfigHandler.mudLevel + "\"");
	...
	public static final void register(final EntityPlayer player) {
		...
		player.registerExtendedProperties(propertyTagName, new PlayerMudManager(player));
	}
	...
	public void saveNBTData(final NBTTagCompound nbtCompound) {
		...
		nbtTag.setInteger(propertyMudLevelName,
			this.player.getDataWatcher().getWatchableObjectInt(PlayerMudManager.mudControlDatawatcher));
		nbtCompound.setTag(propertyTagName, nbtTag);
	}
	...
	public void loadNBTData(final NBTTagCompound nbtCompound) {
		...
		this.player.getDataWatcher().updateObject(PlayerMudManager.mudControlDatawatcher,
			(Object) nbtTag.getInteger(propertyMudLevelName));
	}
	...
	public void setMudLevel(final int mudLevel) {
		...
		this.player.getDataWatcher().updateObject(PlayerMudManager.mudControlDatawatcher,
			(Object) (datawatcher / 65536 * 65536 | mudLevel * 256 | (datawatcher & 0xFF)));
	}
	...
	public void setMudTime(final int mudTime) {
		...
		this.player.getDataWatcher().updateObject(PlayerMudManager.mudControlDatawatcher,
			(Object) ((this.getMudControlDatawatcher() & 0xFFFF) | mudTime * 65536));
	}
	...
	public void setMudType(final int mudType) {
		...
		this.player.getDataWatcher().updateObject(PlayerMudManager.mudControlDatawatcher,
			(Object) (this.getMudControlDatawatcher() / 256 * 256 | mudType));
	}
	...
	public void addMudTime(final int mudTime) {
		...
		final int datawatcher = this.getMudControlDatawatcher();
		this.player.getDataWatcher().updateObject(PlayerMudManager.mudControlDatawatcher,
			(Object) ((datawatcher & 0xFFFF) | (mudTime + datawatcher / 65536) * 65536));
	}
}
