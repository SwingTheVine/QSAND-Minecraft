...
public class SkinOverlayRenderer extends ModelRenderer {
	...
	public SkinOverlayRenderer(final ModelBase model, final EntityPlayer player) {
		...
		this.playerNickname = player.getName(); // Sets the command line name of the player to the command line name for this instance
	...
	public void render(final float translationAmplifer) {
        ...
        if (mudTime > 50 && mudType >= 0 && mudType <= QSAND_Blocks.blockObjectList.length / 5 && mudLevel > 0) {
            ...
            if (this.player instanceof AbstractClientPlayer) {
                Minecraft.getMinecraft().renderEngine.bindTexture(((AbstractClientPlayer)this.player).getLocationSkin());
            }
            else {
                Minecraft.getMinecraft().renderEngine.bindTexture(((EntityOtherPlayerMP)this.player).getLocationSkin());
            }
